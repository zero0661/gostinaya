...
- в английской навигации пункт `Audio` расположен между `Project Map` и `About the Project` и ведёт на `/en-audio-essays/`; русская навигация не менялась.
...
Английская страница аудиоэссе опубликована со slug `en-audio-essays`. Точная
копия её стартового содержимого, глобального и страничного code injection
хранится в `docs/GHOST-EN-AUDIO-NAVIGATION.md`.

### 13.4. Разделение авторских профилей RU/EN и финальные блоки страниц

В Ghost действуют два отдельных авторских профиля:

- `Пётр Миленин` — русские материалы;
- `Peter Milenin` — английские материалы, роль `Author`.

На 15 сентября 2026 года профиль `Peter Milenin` назначен единственным автором
18 английских статей с internal tag `English` и шести английских страниц:
`After Login`, `Project Map`, `About the Project`, `About the Author`,
`Audio Essays`, `Contact`. Русский профиль с этих материалов снят.

При публикации из основного административного аккаунта отдельный вход в профиль
автора не требуется: в настройках материала нужно выбрать `Пётр Миленин` для
русской версии и `Peter Milenin` для английской версии.

На русских страницах `Карта проекта`, `О проекте`, `Об авторе`, `Аудиотека` и
`Контакты` прежний финальный баннер заменён блоком того же типа, который
используется в русских статьях: подписка, приглашение «Разговор продолжается в
Гостиной» и контакт автора. Кнопка страницы ведёт на `/gostinaya/`. Английские
страницы при этой операции не изменялись.

Назначения авторов и содержимое страниц хранятся в базе Ghost, а не в этом
репозитории. Для точного восстановления нужны дамп Ghost/MariaDB и архив Ghost
content volume. Контрольный перечень и порядок проверки находятся в
`docs/GHOST-AUTHORS-AND-RU-PAGE-CTA.md`.

### 13.5. Английские bookmark-карточки

На странице `Project Map` (`/en-project-map/`) внутренние метаданные всех 20
bookmark-карточек приведены к английскому виду:

- publisher: `>_ AFTER LOGIN`;
- author: `Peter Milenin`.

Ссылки, заголовки, описания и изображения карточек при замене не изменялись.
После публикации проверены все 20 карточек в редакторе Ghost после перезагрузки
и на публичной странице; русских publisher/author среди них не осталось.

Дополнительно в Ghost Site footer code injection установлен защитный скрипт,
который локализует эти два поля в любых bookmark-карточках на английских URL
`/en/…` и `/en-…`. На русских URL скрипт ничего не меняет. Каноническая копия
скрипта и порядок проверки находятся в `docs/GHOST-EN-BOOKMARK-CARDS.md`.

Содержимое `Project Map` хранится в базе Ghost/MariaDB. Git содержит описание
проверенного состояния и копию code injection, но не заменяет SQL dump Ghost.
...
Первое сообщение темы и ответы хранятся в `discussion_messages`. Ответ на конкретное сообщение получает `parent_message_id`. Пользовательский текст выводится как экранированный текст; адреса с протоколами `http://` и `https://` преобразуются функцией `utils/linkifyText.js` в безопасные внешние ссылки с `noopener`, `noreferrer` и `nofollow`. Полноценный HTML-редактор не используется.
...
| 15 сентября | отдельный профиль `Peter Milenin` для всех английских материалов; единый финальный блок на пяти русских служебных страницах; английская страница `Audio Essays` и пункт `Audio` в английском меню; английские publisher/author зафиксированы внутри 20 bookmark-карточек `Project Map` и защищены site footer code injection |

История находится в Git; исходная точка — 6 июля 2026 года, последняя
зафиксированная в этом паспорте конфигурация Ghost — 15 сентября 2026 года.
